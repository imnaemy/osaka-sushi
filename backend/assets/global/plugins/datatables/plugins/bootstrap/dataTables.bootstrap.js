...
$.extend($.fn.dataTableExt.oPagination, {
    "bootstrap_extended": {
        "fnInit": function (oSettings, nPaging, fnDraw) {
            ...
            $(nPaging).append(
                '<div class="pagination-admin"> ' + oLang.page + ' ' +
                '<a href="#" class="btn btn-sm default prev disabled" title="' + oLang.previous + '"><i class="fa fa-angle-left"></i></a>' +
                '<input type="text" class="pagination-admin-input form-control input-mini input-inline input-sm" maxlenght="5" style="text-align:center; margin: 0 5px;">' +
                '<a href="#" class="btn btn-sm default next disabled" title="' + oLang.next + '"><i class="fa fa-angle-right"></i></a> ' +
                oLang.pageOf + ' <span class="pagination-admin-total"></span>' +
                '</div>'
            );
            ...
            $('.pagination-admin-input', nPaging).bind('change.DT', function (e) {
                var oPaging = oSettings.oInstance.fnPagingInfo();
                e.preventDefault();
            ...
            $('.pagination-admin-input', nPaging).bind('keypress.DT', function (e) {
                var oPaging = oSettings.oInstance.fnPagingInfo();
                if (e.which == 13) {
                    var page = parseInt($(this).val());
        ...
        "fnUpdate": function (oSettings, fnDraw) {
            ...
            for (i = 0, iLen = an.length; i < iLen; i++) {
                ...
                $('.pagination-admin-total', an[i]).html(oPaging.iTotalPages);
                $('.pagination-admin-input', an[i]).val(oPaging.iPage + 1);
